...
def handle_trades(current_price):
    global capital,bot_total_volume,bot_total_coins,dca_bo,dca_so,dca_mstc,dca_os,dca_tp,dca_sos,dca_ss,last_bought_price,dca_current_so,dca_so_count,dca_last_so,next_so_buy_price,bot_avg_price,sell_price,dca_fee
    if bot_total_volume==0:
        ...
        capital = capital - (dca_bo/100*dca_fee)
        ...
        if current_price <=next_so_buy_price:
            if dca_current_so <dca_mstc:
                if dca_current_so ==0:
                    ...
                    capital = capital - (dca_so/100*dca_fee)
                    dca_last_so = dca_so
                    bot_total_coins += buy_amount
                    next_so_buy_price = current_price - (current_price / 100 * dca_sos)
                    bot_total_volume = bot_total_volume + (buy_amount * current_price)
                    ...
                    capital = capital - (dca_last_so/100*dca_fee)
